...
function updatePasscodeDots() {
  const length = passwordInput?.value.length ?? 0;
  ...
  dotsWrap?.setAttribute("aria-label", `${length} dari ${CONFIG.accessPassword.length} digit telah dimasukkan`);
}

function clearPasscode(animate = false) {
...
function validatePasscode() {
  ...
  if (enteredPassword === CONFIG.accessPassword) {
    ...
    document.getElementById("passcodeDots")?.classList.add("success");

    window.setTimeout(() => {
      clearPasscode();
      document.getElementById("passcodeDots")?.classList.remove("success");
      setPasswordMessage();
      showScene("letter");
    }, 550);
    return;
...
function appendPasscodeDigit(digit) {
  if (!passwordInput || passwordInput.value.length >= CONFIG.accessPassword.length) return;
  passwordInput.value += digit;
  setPasswordMessage();
  updatePasscodeDots();

  if (passwordInput.value.length === CONFIG.accessPassword.length) {
    window.setTimeout(validatePasscode, 120);
  }
}
...
numericKeypad?.addEventListener("click", (event) => {
  ...
  if (!key) return;
  appendPasscodeDigit(key.dataset.key);
});

deletePasscode?.addEventListener("click", removeLastPasscodeDigit);
...
document.addEventListener("keydown", (event) => {
  if (!document.getElementById("passwordGate")?.classList.contains("active")) return;
...
restartButton?.addEventListener("click", () => {
  ...
  passwordAttempts = 0;
  clearPasscode();
  document.getElementById("passcodeDots")?.classList.remove("success", "shake");
  setPasswordMessage();
