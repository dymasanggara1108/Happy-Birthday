...
const passcodeDotsWrap = document.getElementById("passcodeDots");
...
async function playIntroMusic() {
  if (!introSong || !birthdaySong?.paused) return;

  try {
    await introSong.play();
  } catch (error) {
    // Autoplay bersuara dapat diblokir browser. Interaksi pertama akan mencoba lagi.
  }
}
...
function stopIntroMusic(reset = true) {
  ...
  if (reset) {
    try {
      ...
      // Metadata audio mungkin belum siap; aman untuk diabaikan.
    }
  }
}

function unlockIntroMusic() {
  if (introSong?.paused && birthdaySong?.paused) {
    playIntroMusic();
  }
}

// Coba autoplay saat website dibuka, lalu fallback ke interaksi pertama pengguna.
playIntroMusic();
document.addEventListener("pointerdown", unlockIntroMusic, { once: true, capture: true });
document.addEventListener("keydown", unlockIntroMusic, { once: true, capture: true });
...
function renderPasscode() {
  ...
  passcodeDotsWrap?.setAttribute(
    "aria-label",
    `${value.length} dari ${CONFIG.accessPassword.length} digit telah dimasukkan`
  );
}
...
function showCurrentPasswordHint() {
...
function validatePasscode() {
  if (!passwordInput || isPasscodeLocked) return;

  const enteredPassword = passwordInput.value;
  if (enteredPassword.length !== CONFIG.accessPassword.length) return;
  ...
  if (enteredPassword === CONFIG.accessPassword) {
    ...
    passwordForm?.classList.add("passcode-success");

    window.setTimeout(() => {
      clearPasscode();
      setPasswordMessage();
      passwordForm?.classList.remove("passcode-success");
  ...
  passcodeDotsWrap?.classList.remove("shake");
  void passcodeDotsWrap?.offsetWidth;
  passcodeDotsWrap?.classList.add("shake");

  if (passwordAttempts > 3) {
    setPasswordMessage("hari ulang tahunmu", "hint");
  } else {
    setPasswordMessage(
      `Sandi belum tepat. Percobaan ${passwordAttempts} dari 3 sebelum petunjuk muncul.`,
      "error"
    );
  }
...
function appendPasscodeDigit(digit) {
  ...
  if (passwordInput.value.length >= CONFIG.accessPassword.length) return;

  passwordInput.value += digit;
  renderPasscode();

  // Setelah pengguna mulai mengetik ulang, hilangkan pesan error lama.
  if (passwordAttempts <= 3) setPasswordMessage();

  if (passwordInput.value.length === CONFIG.accessPassword.length) {
    window.setTimeout(validatePasscode, 90);
  }
}

function removePasscodeDigit() {
  if (!passwordInput || isPasscodeLocked || passwordInput.value.length === 0) return;
...
keypadKeys.forEach((button) => {
  button.addEventListener("click", () => {
    appendPasscodeDigit(button.dataset.key ?? "");
  });
});

deletePasscode?.addEventListener("click", removePasscodeDigit);
...
// Dukungan keyboard komputer saat scene sandi sedang aktif.
document.addEventListener("keydown", (event) => {
  const passwordScene = document.getElementById("passwordGate");
  if (!passwordScene?.classList.contains("active")) return;

  if (/^\d$/.test(event.key)) {
    event.preventDefault();
    appendPasscodeDigit(event.key);
    return;
  }
...
restartButton?.addEventListener("click", () => {
  ...
  // Restart pengalaman juga memulai ulang musik pembuka dari detik 0.
  stopIntroMusic(true);
  playIntroMusic();
});
